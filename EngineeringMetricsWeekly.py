...
# Todo: Open each file for update or check if automatic when accesing via python

# C:\Users\ggalina\Miniconda3\python.exe "C:\Users\ggalina\Engineering Metrics\EngineeringMetricsWeekly.py"
# Es para correrlo en el cmd
...
df_cck = pd.read_excel(path_sp, sheet_name= 'CCK Data')
filt_ccktime = (df_cck['Time - Complete Information Received'].dt.year == 2020) & (df_cck['Time - Request Completed'].dt.year ==2020)
df_cck1 = (df_cck[filt_ccktime]['Time - Request Completed'].dropna() - df_cck[filt_ccktime]['Time - Complete Information Received'].dropna()).astype('timedelta64[h]')
df_cck1[df_cck1 < 0] = None
c_ck_time = round(df_cck1.dropna().mean(), 1)
